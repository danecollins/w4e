...
class TestSentinel(TestCase):
    def test_create(self):
        x = Sentinel(name='DB backup',
                     tag=id_generator(),
                     freq=60)
        x.save()
        self.assertEqual(len(Sentinel.objects.all()), 1)

    def test_string_rep(self):
        x = Sentinel(name='db backup', tag='hello', freq=60, active=True)
        x.save()
        expect = "Sentinel(db backup,hello,60,True)"
        self.assertEqual(expect, str(x))
        expect = "Sentinel(name='db backup', tag='hello', freq=60, active=True)"
